...
class TestOrder(unittest.TestCase):
    ...
    def test_1order(self, expect):
        self.index_proxy.go_to_my_cart()  # 跳转我的购物车
        # self.goods_cart_proxy.go_to_order_check_func()  # 跳转确认订单页面方法
        self.cart_proxy.go_check_order_page()  # 跳转确认订单页面方法
        time.sleep(1)
    ...
    def test_2order_pay(self, expect):
        """订单支付测试"""
        self.index_proxy.go_to_my_order_page()  # 点击订单支付
        # 切换新窗口
        switch_new_window()

        self.my_order_proxy.go_to_payment_page()  # 点击立即支付
        # 切换新窗口
        switch_new_window()
