...
class OrderPayPage(BasePage):
    ...
    def __init__(self):
        super().__init__()
        self.after_pay = ('css selector', "[value='pay_code=cod']")  # 货到付款支付方式
        self.confirm_pay = ('link text', "确认支付方式")  # 确认支付方式
...
class OrderPayHandle(BaseHandle):
    ...
    def click_after_pay(self):
        """货到付款方式点击方法"""
        self.click_func(self.order_pay_page.find_after_pay())

    def click_confirm_pay(self):
        """确认支付点击方法"""
        self.click_func(self.order_pay_page.find_confirm_pay())
